...
export const Select: React.FC<SelectProps> = ({ value, onValueChange, options, placeholder = "Select...", className = "" }) => {
  ...
  return (
    <div className={`relative w-full ${className}`} ref={containerRef} style={{ zIndex: isOpen ? 60 : 1 }}>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 z-50 mt-1 min-w-[8rem] w-full max-h-[160px] overflow-y-auto rounded-md border border-slate-200 bg-white text-slate-950 shadow-xl animate-in fade-in zoom-in-95 duration-100">
          <div className="p-1">
